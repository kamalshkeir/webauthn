...
import (
	...
	"github.com/kamalshkeir/webauthn/metadata"
	"github.com/kamalshkeir/webauthn/protocol/webauthncose"
)
...
func handleECDAAAttestation(signature, clientDataHash, ecdaaKeyID []byte) (string, []any, error) {
	err := fmt.Errorf("handleECDAAAttestation is not implemented %v %v %v", signature, clientDataHash, ecdaaKeyID)
	return "Packed (ECDAA)", nil, errors.Join(ErrNotSpecImplemented, err)
}
